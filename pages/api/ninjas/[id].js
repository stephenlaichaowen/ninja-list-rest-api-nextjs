...
export default async (req, res) => {
  ...
  switch (method) {
    ...
    // case 'PATCH':
      ...
      try {
        let ninjas = JSON.parse(fs.readFileSync(path.resolve('./public/data.json')))        
    ...
    case 'DELETE':
      try {
        ...
        fs.readFile(fs.readFileSync(path.resolve('./public/data.json')), (err, data) => {
          if (err) throw err
          ...
          // fs.writeFile('data.json', JSON.stringify(ninjas, null, 2), (err) => {
          fs.writeFile(fs.readFileSync(path.resolve('./public/data.json')), JSON.stringify(ninjas, null, 2), (err) => {
            if (err) throw err;
            console.log('Data written to file');
          });
